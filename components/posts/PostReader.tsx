...
function CommentItem({
  ...
  return (
    <div className={`space-y-3 ${depth === 0 ? 'p-4 bg-[#070a0f] border border-slate-800/60 rounded-xl' : 'p-3 bg-[#0b0f17] border border-slate-800/50 rounded-xl'}`}>
      {/* Comment Header */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <img
            ...
            className={`${depth === 0 ? 'w-7 h-7' : 'w-5 h-5'} rounded-full object-cover border border-slate-700/80`}
          />
          <span className="text-xs font-bold text-white tracking-tight">{comment.authorName}</span>
          {parentComment && (
            <span className="text-[10px] text-emerald-400/80 font-mono">@{parentComment.authorName}</span>
          )}
        </div>
        <span className="text-[10px] text-slate-500 font-mono">{new Date(comment.createdAt).toLocaleDateString()}</span>
      </div>

      {/* Comment Content */}
      <p className={`text-xs ${depth === 0 ? 'sm:text-sm' : ''} text-slate-300 leading-relaxed ${depth === 0 ? 'pl-9' : 'pl-7'}`}>
        {comment.content}
      </p>

      {/* Actions */}
      <div className={`${depth === 0 ? 'pl-9' : 'pl-7'} flex items-center gap-4`}>
        <button
          type="button"
          onClick={() => {
            if (!currentUser) {
          ...
          className="text-[11px] font-semibold text-emerald-400/90 hover:text-emerald-300 flex items-center gap-1 transition"
        >
      ...
      {/* Inline Reply Form */}
      {isReplying && (
        <div className={`mt-3 ${depth === 0 ? 'pl-3 sm:pl-9' : 'pl-3 sm:pl-7'}`}>
          <form
            onSubmit={(e) => {
              e.preventDefault()
            ...
            className="p-3 bg-[#070a0f] border border-emerald-500/30 rounded-xl space-y-2.5"
          >
            <div className="flex items-center justify-between text-[11px] text-slate-400">
              <span className="flex items-center gap-1 text-emerald-400 font-medium">
                Replying to <strong className="text-white">@{replyTargetAuthor || comment.authorName}</strong>
              </span>
              <button
                type="button"
      ...
      {childReplies.length > 0 && !isCollapsed && (
        <div className={`mt-3 ${depth < 3 ? 'pl-3 sm:pl-6 border-l-2 border-slate-800/80' : 'pl-2 border-l border-slate-800/50'} space-y-3`}>
